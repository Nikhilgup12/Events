...
class Events extends Component {
  state = {activeEvent: 'INITIAL', activeTab: ''}

  onClickEventImage = (registrationStatus, id) => {
    this.setState({activeEvent: registrationStatus, activeTab: id})
  }

  render() {
    const {activeEvent, activeTab} = this.state
    return (
      <div className="event-main-container">
          ...
          <ul className="event-item">
            {eventsList.map(eachItem => (
              <EventItem
                eventItem={eachItem}
                key={eachItem.id}
                onClickEventImage={this.onClickEventImage}
        ...
        <div className="active-event-container">
          <ActiveEventRegistrationDetails
            contsructEvenet={contsructEvenet}
